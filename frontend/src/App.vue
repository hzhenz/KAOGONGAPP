...
const userList = ref([])
const jobFilters = reactive({ year: '2024', province: '广东', major: '法学' })
...
async function loadOverview() {
  ...
  try {
    ...
    jobResults.value = data.jobResults || []
...
async function loadAdminOverview() {
  ...
  try {
    ...
    jobResults.value = data.jobs || jobResults.value
  } catch (error) {
...
async function searchJobs() {
  ...
  try {
    ...
    jobResults.value = await request(`/jobs?${query}`)
  } catch (error) {
    ...
    <main class="main">
      ...
      <section class="grid three-cols top-align">
        ...
        <div class="card">
          ...
          <div v-for="job in jobResults" :key="`${job.name}-${job.year}`" class="list-item row">
            <div>
      ...
      <section class="grid two-cols">
        <div class="card">
          ...
          <div class="list-item">
            ...
            <p>支持年份、省份、专业三维筛选，并支持管理员新增岗位数据。</p>
          </div>
          <div class="list-item">
            <strong>非功能补充</strong>
            <p>图片默认零留存，上传后仅保留审核结果与元信息，减少隐私残留。</p>
          </div>
        </div>
